"""
Aurum Ghana - Seed data
Run: python -m backend.seed
"""
import os
from backend.app import create_app
from backend.extensions import db
from backend.models import Category, Product, User, Supplier
...
def seed():
    ...
    with app.app_context():
        ...
        supplier_data = {
            "name": "Global Jewelry Supply Co.",
            "country": "China",
            "contact_name": "Wei Chen",
            "contact_email": "orders@globaljewelrysupply.com",
            "contact_phone": "+86 755 1234 5678",
            "address": "Baoan District, Shenzhen, China",
            "api_endpoint": None,
            "api_key": None,
            "notes": "Primary dropship partner. Accepts orders via email. Lead time: 7-14 days. MOQ: 5 units per SKU.",
            "lead_time_days": 10,
            "is_active": True,
        }
        sup = Supplier.query.filter_by(name=supplier_data["name"]).first()
        if not sup:
            sup = Supplier(**supplier_data)
            db.session.add(sup)
            db.session.flush()
        ...
        for cat_slug, name_list in NAMES.items():
            ...
            for name in name_list:
                ...
                product = Product(
                    ...
                    review_count=(i * 3) % 50,
                    fulfillment_mode=("dropship" if (i % 9 == 0) else "in_stock"),
